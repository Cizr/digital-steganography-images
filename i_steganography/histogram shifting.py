...
# Function to compute the histogram of an 8-bit image.
"""
Parameters:
...
# Function to determine the maximum number of bits that a specific channel can handle in an 8-bit cover image.
"""
Parameters:
    cover_image: an 8-bit numpy array representing the cover image
"""
def max_data(cover_image):
    # Verify that the image is 8-bit and then find the histogram of pixel intensities in the image.
    if cover_image.dtype != "uint8":
        return -1
    intensities = histogram_8bit(cover_image)[1]
    # Find the peak point of the histogram to determine the maximum number of bits that can be used for hiding data.
    peak_point = np.argmax(intensities)
    # Return this maximum number of bits.
    return intensities[peak_point]

# Function to hide data in an 8-bit cover image.
"""
Parameters:
    cover_image: an 8-bit numpy array representing the cover image
...
def hide_data(cover_image, bit_stream):
    # Verify if the image is an 8-bit image.
    if cover_image.dtype != "uint8":
        return -1
    ...
    img_hei, img_wid = cover_image.shape[:2]
    for i in range(img_hei):
        for j in range(img_wid):
...
# Function to decode data from an 8-bit cover image.
"""
Parameters:
...
def Image_Encoder(img_2_encr, txt_2_encr):
    # Load the image and split it into its RGB channels.
...
def Image_Decoder(encr_img, encr_data):
    ...
    def Decoder(cover, peak, size):
        bit_list = reveal_data(cover, peak, size)
        return bit_list
    ...
    for i, d in enumerate(data):
        if (d[0] > 0) and (d[1] > 0):
            ...
            bit_stream.extend(xbs) 
            xbs = []

    with open('decoded.txt', 'w') as decoded:
        decoded.write(''.join(bit_stream))
    print("Image Decoded!")
